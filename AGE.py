...
root.configure(bg="#8EA8FF")   #setting up background color
root.geometry("400x300")    #fixing the size of the window
root.iconbitmap("logo.png") # set Icon
new=Label(root,bg="#B9C5FF")  #declaring a label
...
def age(b_date, b_month, b_year):
    ...
    new=Label(root,text="YOUR AGE \n"+resulty+" YEARS "+resultm+" MONTHS "+ resultd+" DAYS ",fg="#393939",bg="#C2F9FF",borderwidth=6)
...
title_label=Label(root,text="AGE CALCULATOR",borderwidth=10,fg="#003763",bg="#8EA8FF")
title_label.config(font=("Arial Rounded MT Bold",29))
title_label.grid(row=0,column=0,columnspan=3)
label_date=Label(root,text="BIRTH DATE : ",borderwidth=4,fg="#003763",bg="#8EA8FF")
label_date.config(font=("Arial Rounded MT Bold",15))
label_date.grid(row=1,column=0)
label_month=Label(root,text="BIRTH MONTH : ",borderwidth=5,fg="#003763",bg="#8EA8FF")
label_month.config(font=("Arial Rounded MT Bold",15))
label_month.grid(row=2,column=0)
label_year=Label(root,text="BIRTH YEAR : ",borderwidth=9,fg="#003763",bg="#8EA8FF")
...
submit=Button(root,text="GET AGE!!",width=10,anchor=CENTER,command=lambda:age(b_date,b_month,b_year),fg="black",bg="#F1F4FF",borderwidth=5)
submit.grid(row=4,column=0)

#calling clean function in button widget
clear=Button(root,text="CLEAR",width=10,command=lambda:clean(entry_date,entry_month,entry_year),fg="black",bg="#F1F4FF",borderwidth=5)
